...
void produce_outputFile(char* string1, char* string2, char* string3, list_t *list, char* file_name2);
...
void produce_outputFile(char* string1, char* string2, char* string3, list_t *list, char* file_name2){
    ...
    record_t* current = list->first_record;
    if (!current){
    ...
    while(current != NULL){
        ...
        /*copy a current record into the output file if they have a key required*/ 
			 fprintf(output_file, "%s %s %s -->", string1, string2, string3); 
		 ...
		 current = current->nextPointWithinR;
    }
...
int main(int argc, char** argv){
	...
	while (getline(&key, &c, stdin) != -1){
		...
		char* x = strtok(key, " ");
		char* y = strtok(NULL, " "); 
		char* tmpr = strtok(NULL, "");
		char* r = (char*)malloc(sizeof(char)*strlen(tmpr)+1); 
		assert(r != NULL);
		strcpy(r, tmpr); 
		getRidOfNewLine(r);
		centre[0] = atof(x); 
		centre[1] = atof(y); 
		centre[2] = atof(r);
		radius_search(records->current_record, centre, list, depth, &count_comparison);
		printf("%s %s %s -->%d\n", x, y, r, count_comparison); 
		produce_outputFile(x,y,r, list, name2);
		free(list); 
		free(r);
		count_comparison = 0; 
    }
